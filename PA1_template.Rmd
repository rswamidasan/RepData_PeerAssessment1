...
```
We see from the above that out of `r dim(act)[1]` observations in the data set, there are `r sum(is.na(act$steps))` missing values, all in the "steps" column.  Because their proportion is relativley low (`r round(mean(is.na(act[, 1]))*100,2)`%), we choose to ignore the missing values for now.  
...
```
We note from the table above, that there are `r sum(is.na(dsteps$steps))` days with missing data.  From earlier, we know that there are `r sum(is.na(act$steps))` missing values for "steps" in the original data.  Given that there are 288 observations per day, and that 288 x 8 = `r 288 * 8`, this implies that a given date has either all "step" values missing (8 such days) or no missing values at all (53 such days).
...
This gives a bin size = `r rawBinW` and the number of bins = `r rawNBins`.  However, further in this analysis, missing values are imputed and filled in.  A similar calculation done on the data set with missing data filled in with imputed values yields a bin width = 1500, with the number of bins = 15.
...
The total number of steps taken per day has a Mean = `r as.character(round(rawMean, 2))` and a Median = `r rawMed`.  
...
```
```{r line, fig.height = 4, fig.width = 8}
...
```
The 5-minute interval with the maximum number of steps on average, across all days, is `r iSteps[which.max(iSteps$steps),1]`, corresponding to `r timeHM(as.integer(iSteps[which.max(iSteps$steps),3]))` a.m.  The average rate of motion during this peak interval across all days is `r peak` steps in 5 minutes.  
...
```
dcact is a `r dcrows` x `r dccols` data frame (interval x date) where:  
...
```
As explained earlier, although the Freedman-Diaconis rule yields a bin width = `r newBinW`, we use a bin width = `r binW`, as in the earlier histogram, to make a meaningful comparison between raw and imputed data set characteristics.  The new mean and meadian, and the means for weekend and weekday data are calculated below.
...
In the new data set the total number of steps taken per day has a Mean = `r as.character(round(newMean, 2))` and a Median = `r as.character(newMed)`.  These are lower that those of the raw data set, Mean = `r as.character(round(rawMean, 2))` and Median = `r rawMed`.  This decrease of `r as.character(round(rawMean - newMean, 2))` in the Mean (and the lower Median) is because we have 6 weekdays imputed with the value `r as.character(round(MeanWD, 2))` (which is less than the raw data Mean by `r as.character(round(diffWD <- (rawMean - MeanWD), 2))`)  and only 2 weekend dates imputed with the value `r as.character(round(MeanWE, 2))` (which is greater than the raw data Mean by `r as.character(round(diffWE <- (MeanWE -rawMean), 2))`).  

The weighted differences are: `r as.character(round(diffWD, 2))` x 6 = `r as.character(round(delWD <- diffWD * 6, 2))` below and `r as.character(round(diffWE, 2))` x 2 = `r as.character(round(delWE <- diffWE * 2, 2))` above the raw data Mean.  Now, `r as.character(round(delWE, 2))` - `r as.character(round(delWD, 2))` = `r as.character(round(delNet <- (delWE - delWD), 2))`.  We divide this by 61 days giving `r as.character(round(delNet/61.0, 2))`, the difference between the Mean with the imputed data and the raw data Mean (allowing for rounding errors at several stages in the processing).
